...
import { useAccount, useReadContract as useReadContractHook } from 'wagmi';
...
export function SystemHealth() {
    ...
    const { data: contractName, isError: isContractError, refetch: refetchName } = useReadContractHook({
    ...
    // 2. Check Admin Role
    const { data: defaultAdminRole } = useReadContractHook({
        address: contractAddress,
        abi: SupplyChainTrackerABI,
        functionName: 'DEFAULT_ADMIN_ROLE',
    });

    const { data: isAdmin, refetch: refetchAdmin } = useReadContractHook({
        address: contractAddress,
        abi: SupplyChainTrackerABI,
    ...
    const { data: contractFabricanteHash, refetch: refetchHash } = useReadContractHook({
        address: contractAddress,
        abi: SupplyChainTrackerABI,
        functionName: 'FABRICANTE_ROLE',
    });

    const { data: roleHash } = useReadContractHook({
        address: contractAddress,
        abi: SupplyChainTrackerABI,
        functionName: 'getRoleHash',
        args: ['FABRICANTE_ROLE']
    }) as { data: `0x${string}` };

    // Use the role hash from contract for comparison
    const comparisonHash = roleHash || contractFabricanteHash;
    ...
    const isHashConsistent = localHashes?.FABRICANTE === comparisonHash;
                ...
                <div className="mt-4 pt-4 border-t">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button
                            ...
                            variant="outline"
                            onClick={async () => {
                                const input = document.getElementById('role-check-input