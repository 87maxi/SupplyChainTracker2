...
async function debugFrontendTransaction() {
  console.log('=== DEBUG FRONTEND TRANSACTION ===');
  ...
  try {
    ...
    const tx = await contractWithSigner.grantRole(roleHash, pendingRequest.address, {
      gasLimit: 200000 // Higher gas limit to ensure success
    });
