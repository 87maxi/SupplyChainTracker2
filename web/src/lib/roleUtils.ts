...
export const getRoleHashes = async (): Promise<RoleMap> => {
  ...
  try {
    ...
    cachedRoleHashes = {
      FABRICANTE: fabricanteRole as `0x${string}`,
      AUDITOR_HW: auditorHwRole as `0x${string}`,
      TECNICO_SW: tecnicoSwRole as `0x${string}`,
      ESCUELA: escuelaRole as `0x${string}`,
      ADMIN: defaultAdminRole as `0x${string}`
    };
